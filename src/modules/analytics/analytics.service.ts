...
export class AnalyticsService {
    ...
    /**
   * Retrieves summary statistics for a link.
   */
    async getLinkAnalytics(linkId: string) {
        ...
        topCountries: topCountries.map((c) => ({ country: c.country, clicks: c._count.id })),
        topBrowsers: topBrowsers.map((b) => ({ browser: b.browser, clicks: b._count.id })),
        topDevices: topDevices.map((d) => ({ device: d.device, clicks: d._count.id })),
        topReferrers: topReferrers.map((r) => ({ referrer: r.referrer, clicks: r._count.id })),
        };
    }
     /**
      * Retrieves hourly/daily click counts for graph charting.
      */
